...
class directed_graph { //Graph Class
	...
	public:
		...
		vertex<T> find_vertex(vector<vertex<T>> vertices, int vertexID);
		vertex<T> get_vertex(int vertexID);
		int edge_cost(int source, int destination);
		vector<int> all_edge_cost();
...
vector<vertex<T>> directed_graph<T>::get_vertices() {
	vector<vertex<T>> v; 
	for (auto x: all_vertices) { //For all vertices3
...
vector<vertex<T>> directed_graph<T>::in_order_traversal(const int& u_id, directed_graph<T>& mst) { 
	...
	vector<int> stack;
	vector<vertex<T>> neighbours;
	vector<vertex<T>> currVertex;

	//currVertex = find(get_vertices(), get_vertices(), u_id);

	// while (!stack.empty() || currVertex.first != -1) {
	// 	neighbours = get_neighbours(currVertex->first);
	// 	//sort(neighbours.begin(), neighbours.end());

	// 	if (currVertex->first != -1) {
	// 		stack.push_back(currVertex->first);
	// 		//make curr = lower weight neighbour
	// 		currVertex = neighbours.at(0);

		
	// 	}

	// 	else {
	// 		currVertex = all_vertices.find(stack.back());
	// 		stack.pop_back();
	// 		//vertices.push_back({currVertex->first, currVertex->second});
	// 		//make curr = higher edge
...
template <typename T> //Done
int directed_graph<T>::edge_cost(int source, int destination) {
	return adj_list[source].find(destination)->second;
}

template <typename T> //Done
vertex<T> directed_graph<T>::find_vertex(vector<vertex<T>> vertices, int vertexID) {
	vertex<T> vertex = {-1,0};
...
template <typename T> //Done
vertex<T> directed_graph<T>::get_vertex(int vertexID) {
	vertex<T> vertex = {-2,0};
	for (auto& vert : all_vertices) {
		if (vert.first == vertexID) {
			return {vert.first, vert.second};
		}
	}
	return vertex;
}
...
template <typename T> //Done
vector<int> directed_graph<T>::all_edge_cost() {
	vector<int> edgesCosts;
	for (int i = 0; i < adj_list.size(); i++) { //Loop over number of adj_list items
		for (auto x : adj_list[i]) { //For each vertex in adj_list
			edgesCosts.push_back(x.second);
		}
	}
	return edgesCosts;
}
